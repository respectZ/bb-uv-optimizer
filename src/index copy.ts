			...
			click: () => {
				...
				const dialog = new Dialog({
					...
					onConfirm(formResult: FormResult) {
						...
						const result = packer.pack(formResult, filteredTextures);
						...
						for (const cube of cubes) {
							for (const resultRect of result.placed) {
								if (cube.uuid === resultRect.uuid) {
									...
									const newU0 =
										resultRect.newLocation.x + (resultRect.invertWidth ? resultRect.width : 0);
									const newV0 =
										resultRect.newLocation.y + (resultRect.invertHeight ? resultRect.height : 0);
									const newU2 =
										resultRect.newLocation.x + (resultRect.invertWidth ? 0 : resultRect.width);
									const newV2 =
										resultRect.newLocation.y + (resultRect.invertHeight ? 0 : resultRect.height);
